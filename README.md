...
- **Multiple profiles + themes** — one shared password; each profile (Alex, Jordan, Lily) has its
  own data, goals, assistant notes, and color theme.
...
**Profiles:** Alex = dark forest (charcoal/pine/`#ff7300`). Jordan (`partner`) = lavender/blush
(violet + magenta, contrast-tuned AA). Lily (`lily`) = clean sky-on-lavender (`#c7b6e2` structure,
`#94bfe9` ring/links, sky `#4888c4` actions). Default "Enterprise" = bone/teal/orange (login +
select-profile screens).
